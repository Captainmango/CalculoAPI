...
public class ExpenseControllerTest {
    ...
    {
        var user = mockUserFactory.makeMockAdmin();

        this.mockMvc.perform(get("/api/v1/admin/expenses").with(user(user))).andExpectAll(
            status().isOk(),
    ...
    public void testOnlyAdminsCanAccessTheAdminIndexRoute() throws Exception
    {
        ...
        this.mockMvc.perform(get("/api/v1/admin/expenses").with(user(user)))
                .andExpect(
                    status().isForbidden()
        );
    }
